...
const arrImpares = randomNumbers.filter((number) => number % 2 != 0);
...
/*crazyNumbers.forEach((number) => {
  if (number < 501) console.log(number);
});*/
...
const numbers = [5, 7, 3];
const arrElevados = numbers.map((number) => number ** 2);
//console.log(arrElevados);
...
const arrFilmesNovos = tarantinoMovies.filter(
  (filmes) => filmes.release < 2000
);
console.log(arrFilmesNovos);
